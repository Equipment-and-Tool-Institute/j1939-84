...
public class SupportedSpnModule {
     ...
     */
    private static final Collection<SpnGroup> REQUIRED_DATA_STREAM_SPNS = Arrays.asList(new SpnGroup[] {
     ...
     */
    private static final Collection<SpnGroup> REQUIRED_DATA_STREAM_SPNS_CI = Arrays.asList(new SpnGroup[] {
     ...
     * Spark Ignition Engine
     */
    private static final Collection<SpnGroup> REQUIRED_DATA_STREAM_SPNS_SI = Arrays.asList(new SpnGroup[] {
     ...
     */
    private static final Collection<SpnGroup> REQUIRED_FREEZE_FRAME_SPNS = Arrays.asList(new SpnGroup[] {
            new SpnGroup(92),
            new SpnGroup(110, 1637, 4076, 4193),
            new SpnGroup(190, 4201, 723, 4202),
            new SpnGroup(512),
     ...
     *
     * @param fuelType the {@link FuelType} of the Engine
     * @return a {@link Collection} of {@link SpnGroup}
     */
    private Collection<SpnGroup> getRequiredDataStreamSpns(FuelType fuelType) {
        Collection<SpnGroup> requiredSpns = new HashSet<>();
        requiredSpns.addAll(REQUIRED_DATA_STREAM_SPNS);
    ...
    public boolean validateDataStreamSpns(ResultsListener listener, Collection<Integer> spns, FuelType fuelType) {
        ...
        for (SpnGroup spnGroup : getRequiredDataStreamSpns(fuelType)) {
            if (!spnGroup.isSatisfied(spns)) {
                listener.onResult("Required Data Stream SPNs are not provided. " + spnGroup);
                result = false;
            }
        }
        return result;
    ...
    public boolean validateFreezeFrameSpns(ResultsListener listener, Collection<Integer> spns) {
        ...
        for (SpnGroup spnGroup : REQUIRED_FREEZE_FRAME_SPNS) {
            if (!spnGroup.isSatisfied(spns)) {
                listener.onResult("Required Freeze Frame SPNs are not provided. " + spnGroup);
                result = false;
            }
        }
        return result;
