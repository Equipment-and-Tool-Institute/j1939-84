...
/**
 * Data stream support verification
 * 6.1.26.1 Actions:
 ...
 * <p>
 * 6.1.26.2 Fail/warn criteria:
 * a. Fail if no response/no valid data for any broadcast SPN indicated as supported by the OBD ECU in DM24.
 * b. Fail if any parameter is not broadcast within ± 10% of the specified broadcast period.
 * c. Fail/warn if any broadcast data is not valid for KOEO conditions as per Table A-1, Minimum Data Stream Support.
 * d. Fail/warn per Table A-1, if an expected SPN from the DM24 support list is provided by a non-OBD ECU.
 * e. Fail/warn per Table A-1 if two or more ECUs provide an SPN listed in Table A-1
 * <p>
 * 6.1.26.3 Actions2
 * a. Identify SPNs provided in the data stream that are listed in Table A-1, but are not supported by any OBD ECU in
 * its DM24 response.
 * 6.1.26.4 Fail/Warn Criteria2
 * a. Fail/warn per Table A-1 column, “Action if SPN provided but not included in DM24”.
 * <p>
 * 6.1.26.5 Actions3:
 * a. DS messages to ECU that indicated support in DM24 for upon request SPNs and SPNs not observed in step 1.
 * b. If no response/no valid data for any SPN requested in 6.1.25.3.a, send global message to request that SPN(s).
 * <p>
 * 6.1.26.6 Fail/warn criteria3: [Refer to footnotes 23 and 24 for a and b, respectively]
 * a. Fail if no response/no valid data for any broadcast SPN indicated as supported by the OBD ECU in DM24.
 * b. Fail if any parameter in a fixed period broadcast message is not broadcast within ± 10% of the specified broadcast
 * period.
 * c. Fail if any parameter in a variable period broadcast message exceeds 110% of its recommended broadcast period.26
 * d. Fail/warn if any broadcast data is not valid for KOEO conditions as per section A.1Table A-1, Minimum Data Stream
 * Support.
 * e. Warn/info per Table A-1, if an expected SPN from the DM24 support list is provided by a non-OBD ECU.
...
public class Step26Controller extends StepController {
    ...
    protected void run() throws Throwable {
        ...
        //This will listen for all Broadcast PGNs in hopes of finding all Data Stream Supported SPNs
        ...
        //6.1.26.3.a. Identify SPNs provided in the data stream that are listed in Table A-1, but are not supported by any OBD ECU in its DM24 response.
        ...
        //We listen for missing SPNs from all modules, rather than waiting for each module
        ...
        for (int moduleAddress : dataRepository.getObdModuleAddresses()) {
            // e. Fail/warn per Table A-1, if an expected SPN from the DM24 support list from an OBD ECU is provided by a non-OBD ECU. (provided extraneously)
        ...
        // d. Fail/warn if any broadcast data is not valid for KOEO conditions as per Table A-1, Minimum Data Stream Support.
    ...
    /**
     * Reports if the given PGN was not received or if any supported SPNs were received as Not Available
     *
     * @param supportedSpns
     ...
     * @return true if the given PGN wasn't received or any supported SPN is Not Available
     */
    ...
    /**
     * Reports the PGNs there are supported by the module but not received and the SPNs that were received by broadcast as Not Available
     *
     * @param obdModuleInformation
     *         the module information
     * @param foundPackets
     *         the Map of PGNs to the List of those packets sent by the module
     * @param spns
    ...
    private List<Integer> collectAndReportNotAvailableSPNs(OBDModuleInformation obdModuleInformation,
            Map<Integer, List<GenericPacket>> foundPackets, List<Integer> spns) {
        ...
        // a. Fail if unsupported (received as not available (as described in SAE J1939-71))
