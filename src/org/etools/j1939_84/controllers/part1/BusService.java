...
public class BusService {
    ...
    /**
     * This sets up the dependency that are necessary at run time (but not available at compile time)
     * This *MUST* be called before interacting with an instance of this class
     */
    ...
    /**
     * Reads the bus for the given number of seconds returning a List of Packets found
     *
     * @param seconds
    ...
    /**
     * Sends a Destination Specific Request for the given PGN to the given moduleAddress
     *
     * @param pgn
     *         the PGN of interest
     * @param moduleAddress
     ...
     * @param supportedSpns
     *         the collection of SPNs which are supported by the module in the data stream
     * @return list of PGNs
     */
    public List<Integer> getPgnsForDSRequest(Collection<Integer> missingSpns, Collection<Integer> supportedSpns) {
        Stream<Integer> onRequestPgns = j1939DaRepository.getPgnDefinitions()
                ...
                .filter(pgnDef -> pgnDef.getSpnDefinitions().stream().anyMatch(s -> supportedSpns.contains(s.getSpnId())))
    ...
    /**
     * Returns a List of PGNs which contain all the given SPNs, if the PGNs is not sent on-request
     */
    ...
    /**
     * Returns a List of PGNs which are contained in the given list and have a broadcast period greater than 0 ms.
     */
