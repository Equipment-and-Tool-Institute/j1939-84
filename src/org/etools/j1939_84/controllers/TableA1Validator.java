...
public class TableA1Validator {
                ...
                if (!invalid.contains(spnId)) {
                    ...
                    if (spn.isError()) {
                        addOutcome(listener,
                                   section,
                                   FAIL,
                                   "N.1 " + moduleName + " reported value for SPN " + spnId
                                           + " (ERROR) is implausible");
                    } else {
                        addOutcome(listener,
                                   section,
                                   WARN,
                                   "N.8 " + moduleName + " reported value for SPN " + spnId + " (" + value
                                           + ") is implausible");
                    }
                    invalid.add(spnId);
    ...
    public void reportPacketIfNotReported(GenericPacket packet, ResultsListener listener, boolean forceReporting) {
        if (!isReported(packet) || forceReporting) {
            ...
            Collection<Integer> spns = dataRepository.isObdModule(moduleAddress)
                    ? dataRepository.getModuleSupportedSPNs(moduleAddress)
                    : getAllSupportedSPNs();
    ...
    public void reportProvidedButNotSupportedSPNs(GenericPacket packet,
        ...
        Collection<Integer> providedSPNs = packet.getSpns()
                                                 ...
                                                 .map(Spn::getId)
                                                 .filter(s -> !dataRepository.getModuleSupportedSPNs(sourceAddress)
                                                                             .contains(s))
