...
public class Part01Step14Controller extends StepController {
    ...
    protected void run() throws Throwable {
        ...
        if (!globalResponse.getPackets().isEmpty()) {
            List<DM26TripDiagnosticReadinessPacket> obdModulePackets = globalResponse.getPackets()
                                                                                     .stream()
                                                                                     .filter(p -> getDataRepository().isObdModule(p.getSourceAddress()))
                                                                                     .peek(p -> {
                                                                                         // 6.1.14.1.a.i. Create list by
                                                                                         // ECU address of all data and
                                                                                         // current status for use later
                                                                                         // in the test.
                                ...
                                if (dm5System != null) {
                                    ...
                                    if (!dm26SystemEnabled && dm5SystemEnabled) {
                                        if (dm26System.getId() == CompositeSystem.COMPREHENSIVE_COMPONENT) {
                                            ...
                                            // monitor
                                            // in DM5 report '0=monitor disabled for rest of this cycle or not
                                            // supported'
                                            // in SPN 3303 bit 3.
            ...
            checkForNACKsGlobal(globalResponse.getPackets(), dsAcks, "6.1.14.5.b");
